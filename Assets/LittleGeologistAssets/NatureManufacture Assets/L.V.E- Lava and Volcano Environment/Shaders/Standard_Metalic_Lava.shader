Shader "NatureManufacture Shaders/Lava/Standard Metalic Lava"
{
	Properties
	{
		...
		_MetalicRAmbientOcclusionGEmissionA("Metalic (R) Ambient Occlusion (G)  Emission(A)", 2D) = "white" {}
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			half4 tex2DNode2 = tex2D( _MetalicRAmbientOcclusionGEmissionA, uv_MainTex );
			...
			float2 uv_LavaNoiseR = i.uv_texcoord * _LavaNoiseR_ST.xy + _LavaNoiseR_ST.zw;
			float2 panner509 = ( _SinTime.x * ( _LavaNoiseSpeed * float2( -1.2,-0.9 ) ) + uv_LavaNoiseR);
			float2 panner510 = ( _SinTime.x * _LavaNoiseSpeed + uv_LavaNoiseR);
